...
- The pattern-dedup path: `tracker/pattern_detector.py` `PatternCompressor.compress_patterns`
  (`:838-867`) produces `compressed_data` + `pattern_refs`. (The former `PatternExporter`/
  ...
  (`tracker/pattern_detector.py:406`), and confirm every detected pattern's occurrences are
  ...
  `sorted(set(exact_matches))` (`pattern_detector.py:283`, `:312`), while a separate
  `occupied_positions` (exact + variation positions, defined right alongside at `:284`, `:313`)
  is used *only* to block a different candidate from claiming the same frames during
  non-overlap selection (`:343-347`) and is never persisted. So every stored reference now
  points at a true exact repeat of the pattern's `events` (variation positions no longer leak
  in). Any note/volume mismatch found by your own round-trip is still CRITICAL; still flag any
  change that starts having an exporter reconstruct from `references` (#4).
...
- `ParallelPatternDetector.detect_patterns` (`tracker/pattern_detector_parallel.py:37`),
  its `_empty_result` (`pattern_detector_parallel.py:344-353`), and the no-events early return
  (`pattern_detector_parallel.py:41-50`).
- `EnhancedPatternDetector.detect_patterns` (`tracker/pattern_detector.py:406`) and its
  no-events return (`pattern_detector.py:410-418`).
  ...
  (`tracker/pattern_detector.py:13`) and a regression test asserting it's gone
  (`tests/test_patterns.py:1314-1320`). This resolved P-08/#105's race/shape concerns by
  construction; #105 and #102 are both **closed**.)
- The `--no-patterns` stub, now built in `detect_patterns_or_direct_export`
  (`main.py:1149`, extracted out of `run_full_pipeline` by #406): confirmed
  **fixed** (#104) — its `stats` keys (`original_size`, `compressed_size`, `compression_ratio`,
  ...
  (#258/PAT-09): the stub now emits `'variations': {}` (`main.py:1193`), matching the 4-key
  envelope both detectors return. A sibling gap survived on the **on-disk** `detect-patterns`
  subcommand file (not this in-memory stub) until #498/PAT-2026-08-23-1 fixed it too
  (`main.py:834-839`, the persisted `output` dict now includes `'variations'`). PAT-06 (#172) is
  likewise **fixed & closed**: the two `_get_variation_summary` implementations now emit the
  SAME inner shape (`pattern_detector.py:526-543` and `pattern_detector_parallel.py:326-342`
  both return `{variation_count, exact_match_count, transposition_range, volume_range}`); the
  parallel path reports `variation_count = 0` and neutral `(0, 0)` ranges by design (exact
  repeats only), not a divergent key set. Confirm both shapes stay unified before trusting any
  consumer of `variations`.
...
- `PatternCompressor.compress_patterns` (`pattern_detector.py:838-867`) fills `pattern_refs`
...
- The old "occurrence-index passed as pattern offset" bug (#102-adjacent, formerly described
  ...
  no longer hardcodes one. `export_frames_and_resolve_mapper` (`main.py:1274`, the stage helper
  #406 extracted out of `run_full_pipeline`) passes the real `pattern_result['references']`
  (`main.py:1315-1318`), deliberately mirroring `run_export` rather than a hardcoded `{}` so the
  two entry points stay forward-compatible if `references` is ever wired up (#379/
  PIPE-2026-07-19-3 — the reasoning is in the comment at `main.py:1305-1312`). The step-by-step
  `run_export` path (`main.py:694-723`) forwards whatever `references` a `detect-patterns` JSON
  ...
  argument is **not consumed** (`exporter/exporter_ca65.py:1628-1648`, #4, closed): `patterns`
...
- Overlap accounting: `_select_best_patterns` (`pattern_detector_parallel.py:286-324`) and the
  selection loop in `pattern_detector.py:324-347` mark `range(pos, pos+length)` used. Confirm
...
`PatternCompressor.calculate_compression_stats` (`tracker/pattern_detector.py:882-930`)
...
reduction" convention — `main.py:850` (subcommand), `main.py:1517` (ROM success banner), and the
`--no-patterns` stub's `0` (`main.py:1183`) is coherent with that convention (0% reduction, not
a misleading `1.0`).
PAT-03 (#169) is now **fixed & closed**: `calculate_compression_stats` takes a `total_events`
arg (`pattern_detector.py:882-883`) and reports a separate `coverage_ratio =
patterned_events / total_events` (`:915-917`), and both banners now print the dedup ratio
explicitly labeled as within the patterned subset PLUS a distinct "Pattern coverage" line
(subcommand `main.py:850`/`:859`, ROM banner `main.py:1517`/`:1519`). The dedup ratio still has
...
- **Scoring**: both `ParallelPatternDetector`'s `_select_candidates_from_groups`
  (`pattern_detector_parallel.py:426`) and `EnhancedPatternDetector.detect_patterns`
  (`pattern_detector.py:266`, `:300`) now call the same module-level `score_pattern`
  (`pattern_detector.py:65`), the parallel path always passing `variation_count=0`. This is
...
- PAT-05 (#171) is now **fixed & closed** — but the fix was a *doc correction*, not a behavior
  ...
  *correctly documents this non-equivalence* (`pattern_detector_parallel.py:451-459`) instead of
  overclaiming equivalence. Confirm the docstring still owns this caveat and hasn't regressed to
  an equivalence claim.
- **Fallback firing**: `run_full_pipeline`'s `detect_patterns_or_direct_export` wraps
  `ParallelPatternDetector` in `try/except` and falls back to `EnhancedPatternDetector`
  (`main.py:1227-1242`), and inside the parallel detector `_detect_patterns_parallel` catches a
  pool-wide failure and calls `_detect_patterns_serial` (`pattern_detector_parallel.py:237-240`).
  Verify the inner serial fallback returns the raw `patterns` dict via `_select_best_patterns`
  (`pattern_detector_parallel.py:284`) — NOT the full `{patterns,references,stats,variations}`
  envelope — and that its caller (`detect_patterns`, `pattern_detector_parallel.py:86-104`) still
  wraps it via the compressor. If the serial fallback's return shape ever bypasses
  compression/stats, the fallback "fires" but yields a malformed result (HIGH).
- P-09 (#106) is now **fixed & closed**: the per-chunk `except` inside the
  *whole-pool-succeeded* path (`pattern_detector_parallel.py:217-231`) no longer silently drops
  a failed chunk's candidates — it recovers that length in-process via `_collect_length_candidates`,
  and only a length that ALSO fails the serial retry is recorded in `failed_subchunks` and
  surfaced by a durable end-of-run warning (`pattern_detector_parallel.py:258-262`), not just a
  transient `pbar.write`. Confirm both the in-process retry and the persistent warning are still
  present.
- The outer `main.py` fallback re-trims events to `max_events` (default `DETECTOR_MAX_EVENTS` =
  300, a named constant, recalibrated down from 1000 by #459/TD-39 — not an ad-hoc "2000" as in
  older notes) via `sample_events_for_detection` (`main.py:1242`) — confirm this matches the
  sequential detector's own internal cap (`pattern_detector.py:232-235`) so the warning reports
  the count actually retained, not a larger figure the detector would silently re-cut (#100,
  closed).
...
- `_detect_patterns_parallel` (`pattern_detector_parallel.py:148-267`) uses work chunks built by
  `_build_work_chunks` — one **tiny** `{'pattern_length', 'start_range'}` sub-chunk per
  pattern-length/start-range slice (`pattern_detector_parallel.py:116-147`) — and ships the
  (potentially large) `sequence` to each worker **once** via the
  `ProcessPoolExecutor(initializer=_init_pattern_worker, initargs=(sequence,))` call (`:200-201`),
  stashed as the module global `_WORKER_SEQUENCE` (`:366-373`) instead of being re-pickled per
  chunk × per length. `valid_events` stays in the parent process — since the #332/PERF-12
  O(n)-hash-grouping rewrite, worker processes (`_detect_window_groups_worker`, `:468-477`) only
  bucket window positions by value; candidate selection (`_select_candidates_from_groups`, the
  step that needs `events`) runs back in the parent against its own `valid_events` (`:244-253`).
  A `_WORKER_EVENTS` global used to also ship `valid_events` to every worker for the old
  candidate-building worker entry point that rewrite replaced — dead weight pickled to every
  spawned process for nothing, fixed in #438/PAT-2026-08-21-4 (`grep -n _WORKER_EVENTS` now
  returns nothing). This directly fixes the old memory-blowup / pickle-cost concern — confirm it
  stays true (no code re-introduces per-chunk copies of the full sequence, and no events payload
  creeps back into `initargs` without a worker that actually reads it). Everything shipped
  through `initargs` here is a plain list of tuples — confirm nothing non-picklable (a
  `tempo_map`, a closure, a numpy array) is added to that call in future changes, since
  `EnhancedTempoMap` itself is deliberately kept out of the worker payload.
- Confirm `_detect_window_groups_worker` (`pattern_detector_parallel.py:468-477`) and
  `_init_pattern_worker` (`:369-373`) are module-level (picklable/importable by the child
  ...
  (`:376-439`) — the actual grouping/candidate-selection work — take `sequence`/`events` as
...
- The dead `ThreadedPatternDetector` (formerly a second, thread-based, shared-`patterns`-dict
  ...
  only a doc comment in `pattern_detector.py:13` and a regression test
  (`tests/test_patterns.py:1314-1320`) asserting it stays gone. No shared mutable state remains
  to audit here.
...
- The advisory `large_file_threshold` (default `LARGE_FILE_THRESHOLD_DEFAULT = MAX_PATTERN_EVENTS`,
  `main.py:56`, overridable via config per #219/#334 — see `get_pattern_detection_caps`,
  `main.py:58-86`), checked at `main.py:1221`, only prints advice — confirm it does not itself
  drop events.
- Both detectors now call the same module-level `sample_events_for_detection`
  (`tracker/pattern_detector.py:39-51`, uniform `np.linspace` sampling — not a head cut).
  `ParallelPatternDetector.detect_patterns` samples to `MAX_PATTERN_EVENTS = 15000`
  (`pattern_detector.py:17`); the sequential `PatternDetector.detect_patterns`
  (`pattern_detector.py:232-235`) samples to `DETECTOR_MAX_EVENTS = 300`
  (`pattern_detector.py:36`, lowered from 1000 by #459/TD-39) because it is O(n²)-ish. Trace
  whether the sampled `valid_events` is what later feeds `references`/frame reconstruction:
  since `export_tables_with_patterns` derives all bytes from `frames` (not from the sampled
  detection sequence, #4), the exported song is NOT altered by this sampling — only
  pattern-detection *quality* is reduced. Confirm this remains true; if a future change makes
  export consume the sampled sequence instead of `frames`, that would become CRITICAL data loss.
- The old third limit (the removed `ThreadedPatternDetector`'s 2000-stride) is gone along with
  ...
  `MAX_PATTERN_EVENTS`/`DETECTOR_MAX_EVENTS` doc comment (`pattern_detector.py:9-36`) that
...
- `min_pattern_length`/`max_pattern_length` constructor defaults are `3..32` for both detectors
  (`PatternDetector.__init__`, `pattern_detector.py:107`; `ParallelPatternDetector.__init__`,
  `pattern_detector_parallel.py:23`), but all three real entry points now consistently override
  `max_pattern_length` to the same named constants — `PATTERN_MIN_LENGTH = 3` /
  `PATTERN_MAX_LENGTH = 12` (`constants.py:18-19`, imported at `main.py:51`) — used by
  `run_detect_patterns` (`main.py:807-809`), the default parallel path (`main.py:1229`), and its
  sequential fallback (`main.py:1237`). This resolves the old "`detect-patterns` alone uses a
  default max of 32" drift; confirm all three call sites still reference the shared constants
  rather than a hardcoded literal before trusting this. Bounds are honored via
...
- `_find_pattern_matches` (`pattern_detector.py:361-379`, sequential detector only — the
  ...
  **fixed & closed**): the scan for matches after the anchor starts at `start_pos + pattern_len`
  (`:371`), not `start_pos + 1`, so a self-similar run (period < pattern length) can no longer
  ...
  non-overlap invariant. Compare against the parallel detector's greedy `next_free` logic inside
  `_select_candidates_from_groups` (`pattern_detector_parallel.py:396-439`). PAT-05 (#171,
  closed, see Dimension 5) documents the residual case where the parallel path's
  whole-candidate rejection can still lose valid later occurrences the sequential scan would
  recover.
- **#365 (PAT-A) is CLOSED**: `score_pattern`'s `>=MIN_PATTERN_OCCURRENCES` gate counts exact +
  ...
  (`pattern_detector.py:324-347`) now also requires `len(candidate['positions']) >=
  MIN_PATTERN_OCCURRENCES` (`:335`, the same named constant `score_pattern` gates on) and
...
- PAT-07 (#173) is now **fixed & closed**: `PatternCompressor._hash_pattern`
  (`pattern_detector.py:870-880`) returns the exact `Tuple[Tuple[int, int], ...]` of `(note,
  volume)` events (matching an updated type hint), NOT `hash()` of it. Used as the sole dedup
  key in `pattern_hash_map` (`compress_patterns`, `pattern_detector.py:846-858`) with no
  ...
  (The former `_optimize_patterns` dead-code helper that duplicated this selection logic with an
  unshared, diverging formula — `pattern_detector.py:382-419` in earlier trees — was deleted
  entirely by #495/PAT-2026-08-23-2; if a similar helper reappears, confirm it's either wired to
  `score_pattern`/`MIN_PATTERN_OCCURRENCES` or removed again.)
...
table guards it at `loop_manager.py:98`, `160`, but a malformed loop dict elsewhere may not).
Verify `generate_jump_table` keys on `loop_info['end']` (`loop_manager.py:103`, `163`): two
...
(`f"loop_{end}_{start}"`, `:141`) that matches what `generate_jump_table` reads back
(`loop_manager.py:167`) — a key-format drift yields `None` tempo_state silently. Since loop
...
`tests/test_enhanced_loop_patterns.py`. Also note the sibling, off-pipeline
`parse_midi_to_frames_with_analysis` path (`tracker/parser_fast.py`), which used to feed loop
detection sampled-space positions over the full event list — fixed by #436/PAT-2026-08-21-2
(the detector there is now sized to `max_events=len(note_on_events)` so it never samples).
...
- [ ] Does `references`/`patterns` stay a pure boolean switch into the exporter with zero consumption of the actual reference values (`exporter/exporter_ca65.py:1628-1648`, #4)? (PAT-01, #168, is fixed so `references` is exact-only, but the exporter still must not consume it.)
