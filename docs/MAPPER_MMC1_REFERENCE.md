...
(`generate_init_code` writes `$0C` to Control). DPCM on MMC1 is **not implemented**;
the Mode-2 `$C000` streaming design that would add it is documented as a future
target at the end of this section.
