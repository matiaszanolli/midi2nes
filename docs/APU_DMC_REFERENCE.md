...
*   **255-Distinct-Sample Ceiling Per Song:** `NESEmulatorCore.process_all_tracks` remaps the DPCM samples a song actually references to a dense, song-local `0..N-1` id (`dense_id`) so a single frame byte (`note = min(255, dense_id + 1)`) can address them without colliding with the shipped catalog's up-to-1922 raw ids. That remap itself has a byte ceiling: at 256+ distinct samples in one song, `dense_id = 255` also encodes to `note = 255`, colliding with `dense_id = 254` — every `dense_id >= 255` silently plays the 255th sample instead of its own (#343/DP-DPCM-04, warned but not prevented). Musically this ceiling is rarely reached; keep a song under 255 distinct DPCM samples to stay exact. **This 255 figure is the frame-generation ceiling only** — with pattern compression on (the default pipeline), the macro-bytecode exporter's own note-dispatch format (`docs/AUDIO_BYTECODE_SPEC.md` §3, `$00`-`$5F`) is the binding limit: a DPCM `note` >= `$60` (`dense_id` >= 95) now raises `ValueError` at export time rather than silently desyncing the bytecode stream (#369/EXP-2026-07-19-1, `exporter/exporter_ca65.py`). `--no-patterns` (direct export) has no such limit and can use the full 255.