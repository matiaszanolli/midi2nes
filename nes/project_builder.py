...
"""
    ...
    def _generate_linker_config(self) -> str:
        ...
        if self.use_mmc1:
            ...
            # Simple single-segment approach for reliability
            return """MEMORY {
    # NES has 2KB of RAM
    ZP:     start = $00,    size = $0100, type = rw, file = "";
    RAM:    start = $0200,  size = $0600, type = rw, file = "";
    
    # NES ROM layout - MMC1 with full 128KB PRG-ROM
    HEADER:  start = $0000,  size = $0010, type = ro, file = %O, fill = yes;
    # Single large PRG segment (128KB minus header and vectors)
    PRG:     start = $0010,  size = $1FFFA, type = ro, file = %O, fill = yes;
    # 6502 vectors at end of ROM
...
SEGMENTS {
    ...
    ZEROPAGE: load = ZP,      type = zp;
    BSS:      load = RAM,     type = bss, define = yes;
    # Put all data and code in single PRG segment
    RODATA:   load = PRG,     type = ro;
    CODE:     load = PRG,     type = ro;
    VECTORS:  load = VECTORS, type = ro;
}"""
